...
useRequestGet('/workshop/info', { id: route.params.id, evaluation: preferences.evaluation })
  .then((result) => {
  ...
  <BaseView :title="$t('submit')">
    <template v-slot:breadcrumb>
      <ElBreadcrumb :separator-icon="ArrowRight" v-if="loaded">
        <ElBreadcrumbItem :to="{ path: '/workshop' }">
          {{ $t('workshop') }}
        </ElBreadcrumbItem>
        <ElBreadcrumbItem :to="{ path: `/work/${$route.params.id}` }">
          {{ title }}
        </ElBreadcrumbItem>
        <ElBreadcrumbItem :to="{ path: `/work/${$route.params.id}/submit` }">
          {{ $t('submit') }}
        </ElBreadcrumbItem>
      </ElBreadcrumb>
    </template>
    <div class="box">
      <SubmitCode :type="1" :problem-id="$route.params.id" />
    </div>
  </BaseView>
</template>
